...
function renderDashboardAuditoria(){
  ...
  const abertas=_dashAudMetas.filter(m=>!['FINALIZADA','ENCERRADA','CANCELADA','EXCLUIDA','EXCLUSAO_PENDENTE'].includes(String(m.status||m.situacao||'').toUpperCase())).length;
  [abertas,total,audit,pend,ok,div,vaz,`${taxa}%`,ops].forEach((v,idx)=>_dashSetKpi(idx,v,['Auditorias em aberto','Endereços previstos','Endereços auditados','Endereços pendentes','Endereços corretos','Divergências','End. vazios','% executado','Operadores'][idx],['🔎','📍','✅','⏳','🎯','⚠️','📭','📊','👥'][idx]));
  const diverg=lista.filter(i=>_dashAudStatus(i)==='DIVERGENTE');
  const produtoLabel=i=>i.produtoEsperado||i.produto_esperado||i.descricaoEsperada||i.descricao_esperada||i.produtoLido||i.produto_lido||i.descricaoProduto||i.descricao_produto||i.dunEsperado||i.dun_esperado||i.gtinEsperado||i.gtin_esperado||i.dunLido||i.dun_lido||i.gtinLido||i.gtin_lido||i.dun||i.gtin||'SEM PRODUTO';
  const enderecoPartes=i=>{const p=_dashEnderecoPartes(i.endereco||i.enderecoCodigo||i.endereco_codigo||'');return {rua:i.rua||i.ruaNome||i.rua_nome||p.rua||'SEM RUA',local:i.local||i.localEstoque||i.local_estoque||i.setor||p.local||'SEM LOCAL',nivel:i.nivel||p.nivel||'SEM NÍVEL',coluna:i.coluna||p.coluna||'SEM COLUNA'};};
  const operadorLabel=i=>i.operadorNome||i.operador_nome||i.nomeOperador||i.nome_operador||i.operadorEmail||i.operador_email||i.operadorId||i.operador_id||'SEM OPERADOR';
  const prod=_dashAgrupar(diverg,produtoLabel);
  const ruas=_dashAgrupar(diverg,i=>enderecoPartes(i).rua), niveis=_dashAgrupar(diverg,i=>enderecoPartes(i).nivel), cols=_dashAgrupar(diverg,i=>enderecoPartes(i).coluna);
  const operadoresDiverg=_dashAgrupar(diverg,operadorLabel);
  const operadoresTotais={};
  lista.filter(i=>_dashAudStatus(i)!=='PENDENTE').forEach(i=>{const nome=operadorLabel(i);if(!operadoresTotais[nome])operadoresTotais[nome]={nome,total:0,ok:0,divergencias:0,vazios:0};const o=operadoresTotais[nome];o.total++;const st=_dashAudStatus(i);if(st==='OK')o.ok++;else if(st==='DIVERGENTE')o.divergencias++;else if(st==='ENDERECO_VAZIO')o.vazios++;});
  const operadores=Object.values(operadoresTotais).map(o=>({...o,produtividade:o.total?Math.round(o.ok/o.total*100):0})).sort((a,b)=>b.total-a.total||b.produtividade-a.produtividade);
  const progressoAgrupado=(campo)=>{const m={};lista.forEach(i=>{const nome=enderecoPartes(i)[campo]||('SEM '+campo.toUpperCase());if(!m[nome])m[nome]={name:nome,total:0,done:0,divergencias:0};m[nome].total++;if(_dashAudStatus(i)!=='PENDENTE')m[nome].done++;if(_dashAudStatus(i)==='DIVERGENTE')m[nome].divergencias++;});return Object.values(m).map(x=>({...x,p:x.total?Math.round(x.done/x.total*100):0})).sort((a,b)=>b.divergencias-a.divergencias||b.p-a.p||a.name.localeCompare(b.name,'pt-BR',{numeric:true}));};
  const progressoRuas=progressoAgrupado('rua'), progressoLocais=progressoAgrupado('local');
  window.DTDashboardAuditoriaSnapshot={lista,diverg,produtos:prod.map(([name,value])=>({name,value})),ruasDivergentes:ruas.map(([name,value])=>({name,value})),operadores,progressoRuas,progressoLocais,total,ok,div,vaz,pend,audit,taxa,acur,abertas};
  ...
  const rwrap=document.getElementById('dash-ruas-wrap');if(rwrap)rwrap.innerHTML=_dashAudBars(operadoresDiverg,'operador',10);
