...
function normalizeRow(row){const map={};Object.keys(row||{}).forEach(k=>map[cab(k)]=row[k]);const pick=arr=>{for(const k of arr){if(map[k]!=null&&txt(map[k]))return txt(map[k]);}return '';};const nome=pick(['PRODUTO','DESCRICAO','NOME DO PRODUTO']);const unidade=pick(['UNIDADE','UN','EMBALAGEM']);const fam=global.DTProdutos.inferirFamilia(nome,unidade);return {codigoInterno:pick(['CODIGO','CODIGO INTERNO','SKU','PRODUTO ID']),nomeProduto:nome,dun:pick(['DUN','CODIGO DUN']),gtin:pick(['GTIN','EAN','CODIGO DE BARRAS']),unidade:unidade,embalagem:fam.embalagem,familiaCodigo:fam.familiaCodigo,familiaNome:fam.familiaNome,produtoPrincipal:fam.familiaNome,ativo:true};}
...
async function salvarProduto(p,id){const agora=new Date().toISOString();const fam=global.DTProdutos.inferirFamilia(p.nomeProduto,p.unidade);const payload={codigoInterno:txt(p.codigoInterno),nomeProduto:txt(p.nomeProduto),dun:txt(p.dun),gtin:txt(p.gtin),unidade:txt(p.unidade),embalagem:txt(p.embalagem)||fam.embalagem,familiaCodigo:txt(p.familiaCodigo)||fam.familiaCodigo,familiaNome:txt(p.familiaNome)||fam.familiaNome,produtoPrincipal:txt(p.produtoPrincipal)||fam.familiaNome,ativo:p.ativo!==false,atualizadoEm:agora,atualizadoPor:global.currentUser?.email||''};if(!id){payload.criadoEm=agora;payload.criadoPor=global.currentUser?.email||'';}await COL().doc(id||idEstavel(payload)).set(payload,{merge:true});}
...
function exportar(){const dados=lista.map(p=>({'Código interno':p.codigoInterno,'Produto':p.nomeProduto,'DUN':p.dun,'GTIN/EAN':p.gtin,'Unidade':p.unidade,'Embalagem':p.embalagem,'Produto principal':p.familiaNome||p.produtoPrincipal,'Status':p.ativo?'ATIVO':'INATIVO'}));const ws=XLSX.utils.json_to_sheet(dados);const wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,ws,'Produtos');XLSX.writeFile(wb,'base_produtos.xlsx');}
