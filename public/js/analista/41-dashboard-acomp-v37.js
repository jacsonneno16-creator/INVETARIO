(function(){
  ...
  window.renderDashboardAuditoria=function(){const lista=window._dashAudFiltrados?window._dashAudFiltrados():[];const total=lista.length,ok=lista.filter(i=>audStatus(i)==='OK').length,div=lista.filter(i=>audStatus(i)==='DIVERGENTE').length,vaz=lista.filter(i=>audStatus(i)==='ENDERECO_VAZIO').length,pend=lista.filter(i=>audStatus(i)==='PENDENTE').length,audit=total-pend,pct=total?Math.round(audit/total*100):0,acur=audit?Math.round(ok/audit*100):0;
    const metas=window._dashAudMetasRef?window._dashAudMetasRef():[];
    const audSel=document.getElementById('dash-faud')?.value||'';
    const abertas=metas.filter(m=>{const st=String(m.status||m.situacao||'LIBERADA').toUpperCase();return !['FINALIZADA','FINALIZADO','CANCELADA','CANCELADO','ENCERRADA','ENCERRADO','EXCLUSAO_PENDENTE'].includes(st)&&(audSel?String(m.id)===String(audSel):true);}).length;
    const ops=new Set(lista.map(i=>i.operadorNome||i.operador_nome||i.operadorId||i.operador_id).filter(Boolean)).size;
    const setKpi=(id,val)=>{const e=document.getElementById(id);if(e)e.textContent=Number.isFinite(val)?Number(val).toLocaleString('pt-BR'):String(val)};
    setKpi('kd-inventarios',abertas);setKpi('kd-enderecos',total);setKpi('kd-end-contados',audit);setKpi('kd-pendencias',pend);setKpi('kd-contagens',ok);setKpi('kd-diverg',div);setKpi('kd-recount',vaz);setKpi('kd-pct-geral',pct+'%');setKpi('kd-operadores',ops);
    window.__DT_DASHBOARD_SNAPSHOT={modo:'auditoria',total,auditados:audit,pendentes:pend,corretos:ok,divergencias:div,vazios:vaz,pct,acuracidade:acur,operadores:ops,auditoriasAbertas:abertas,lista:lista.slice()};
