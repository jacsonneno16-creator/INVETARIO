(function(){
  ...
  window.renderDashboardAuditoria=function(){
    // Primeiro atualiza os KPIs, alerta, tabela e fontes canônicas do dashboard.
    // O renderer moderno abaixo apenas reorganiza a apresentação; não possui
    // uma segunda regra de contagem.
    if(typeof window.renderDashboardAuditoriaBase==='function'){
      window.renderDashboardAuditoriaBase();
    }
    const lista=window._dashAudFiltrados?window._dashAudFiltrados():[];const total=lista.length,ok=lista.filter(i=>audStatus(i)==='OK').length,div=lista.filter(i=>audStatus(i)==='DIVERGENTE').length,vaz=lista.filter(i=>audStatus(i)==='ENDERECO_VAZIO').length,pend=lista.filter(i=>audStatus(i)==='PENDENTE').length,audit=total-pend,pct=total?Math.round(audit/total*100):0,acur=audit?Math.round(ok/audit*100):0;
