...
function buildHeat(){
 ...
 var cells=data.slice(0,72).map(function(x){return '<div class="heat-cell heat-'+heatClass(x.p)+'" title="'+x.name+': '+x.p+'%" data-name="'+x.name+'">'+x.p+'%</div>'}).join('');
 ...
 var rank=data.slice().sort(function(a,b){return b.p-a.p}).slice(0,8).map(function(x,i){return '<div class="ops-rank-row"><b>'+(i+1)+'. '+x.name+'</b><div class="ops-rank-bar"><i style="width:'+x.p+'%"></i></div><strong>'+x.p+'%</strong></div>'}).join('')||'<div style="color:#718078">Sem dados para ranking.</div>';
...
function enhanceDashboard(){var p=qs('#page-dashboard');if(!p)return;var vals=qsa('.kpi-val',p).slice(0,5).map(txt);ensureStrip(p,[card('Inventários ativos',vals[0]||'0','Operações abertas','info'),card('Endereços',vals[1]||'0','Base cadastrada',''),card('Contados',vals[2]||'0','Conferidos','good'),card('Pendências',vals[3]||'0','Aguardando ação','warn'),card('Contagens',vals[4]||'0','Registros processados','')])}
