...
function _garantirBarraFlutuanteRecontagem() {
  let flutuante = document.getElementById('rec-selection-floating');
  if (flutuante) return flutuante;
  const pagina = document.getElementById('page-recontagens');
  if (!pagina) return null;
  flutuante = document.createElement('div');
  flutuante.id = 'rec-selection-floating';
  flutuante.setAttribute('aria-hidden', 'true');
  flutuante.style.cssText = 'display:none;position:fixed;right:24px;bottom:24px;z-index:10050;align-items:center;gap:12px;padding:12px 14px;background:#fff;border:1px solid rgba(232,117,26,.35);border-left:4px solid var(--orange);border-radius:12px;box-shadow:0 14px 38px rgba(15,23,42,.22)';
  flutuante.innerHTML = '<strong id="rec-selection-floating-count" style="font-size:.82rem;color:var(--text)">0 selecionados</strong><button type="button" class="btn btn-primary btn-sm" id="rec-selection-floating-action">👥 Atribuir</button><button type="button" class="btn btn-ghost btn-sm" id="rec-selection-floating-clear">✕</button>';
  pagina.appendChild(flutuante);
  flutuante.querySelector('#rec-selection-floating-action')?.addEventListener('click', function (evento) {
    evento.preventDefault();
    evento.stopPropagation();
    Promise.resolve(abrirAtribuirRecontagem()).catch(function (erro) {
      console.error('[Recontagem] Falha no botão flutuante de atribuição', erro);
      showToast('Não foi possível abrir a atribuição.', 'e');
    });
  });
  flutuante.querySelector('#rec-selection-floating-clear')?.addEventListener('click', function (evento) {
    evento.preventDefault();
    divDeselecionarTodos();
  });
  return flutuante;
}
...
function divAtualizarBarraSel() {
  ...
  const flutuante = _garantirBarraFlutuanteRecontagem();
  const cntFlutuante = document.getElementById('rec-selection-floating-count');
  const quantidade = _divSelecionadas.size;
  const texto = `${quantidade} endereço${quantidade !== 1 ? 's' : ''} selecionado${quantidade !== 1 ? 's' : ''}`;

  if (cnt) cnt.textContent = texto;
  if (cntFlutuante) cntFlutuante.textContent = texto;

  if (bar) {
    bar.style.setProperty('display', quantidade > 0 ? 'flex' : 'none', 'important');
    bar.setAttribute('aria-hidden', quantidade > 0 ? 'false' : 'true');
  }

  if (flutuante) {
    flutuante.style.setProperty('display', quantidade > 0 ? 'flex' : 'none', 'important');
    flutuante.setAttribute('aria-hidden', quantidade > 0 ? 'false' : 'true');
  }

  if (btnTopo) {
    btnTopo.style.setProperty('display', quantidade > 0 ? 'inline-flex' : 'none', 'important');
    btnTopo.disabled = quantidade === 0;
    btnTopo.textContent = quantidade > 0 ? `👥 Atribuir (${quantidade})` : '👥 Atribuir selecionados';
  }
}
...
function divAtribuirRapido(divId) {
  ...
  let div = _obterDivSelecionada(chave);
  if (!div) {
    div = [..._divSelecionaveisRender.values()].find(item =>
      String(item.id || '') === chave ||
      (Array.isArray(item._divergencias_agrupadas) && item._divergencias_agrupadas.map(String).includes(chave))
    ) || null;
  }
  if (!div) {
    console.error('[Recontagem] Linha não encontrada para atribuição', { divId, selecionaveis:[..._divSelecionaveisRender.keys()] });
    showToast('Não foi possível localizar essa recontagem. Clique em Atualizar e tente novamente.', 'e');
    return false;
  }

  if (!divPodeSelecionar(div)) {
    showToast('🔒 Esta atividade já foi iniciada, concluída ou atingiu o limite de contagens.', 'w');
    return false;
  }

  _recAtribuirDireto = null;
  _divSelecionadas.clear();
  _divSelecionadas.add(String(div.id));
  divAtualizarBarraSel();

  Promise.resolve(abrirAtribuirRecontagem()).catch(erro => {
    console.error('[Recontagem] Falha ao abrir atribuição rápida', { divId, div, erro });
              ...
              <div style="display:flex;gap:4px;flex-wrap:wrap">
                ...
                ${(d.status !== 'RESOLVIDA' && d.status !== 'PERSISTENTE')
                  ? (atribPara
                      ...
                      : (!_isFluxoEncerrado(d) ? `<button class="btn btn-ghost btn-sm" style="font-size:.7rem" type="button" data-rec-atribuir="${encodeURIComponent(String(d.id || ''))}" onclick="return divAtribuirRapido(decodeURIComponent(this.dataset.recAtribuir))" title="Atribuir recontagem">👤 Atribuir</button>` : ''))
                  : ''}
              </div>
            </td>
          </tr>`;
