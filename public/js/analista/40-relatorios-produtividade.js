...
(function(global){
  ...
  function renderProdutividadeGraficos(rows,cont){
    ...
    const colunas='<div class="dash-column-legend"><span><i style="background:#2563eb"></i>Endereços</span><span><i style="background:#f97316"></i>Contagens</span></div><div class="dash-column-scroll"><div class="dash-column-chart" style="--chart-count:'+top.length+';min-width:max(520px,calc('+top.length+' * 92px))">'+top.map(function(x,i){const he=Math.max(x.enderecos.size?8:2,Math.round(x.enderecos.size/max*100)),hc=Math.max(x.contagens?8:2,Math.round(x.contagens/max*100)),sel=produtividadeOperadorSelecionado===x.nome;return '<button type="button" class="dash-column-group" onclick="filtrarProdutividadeOperador('+JSON.stringify(x.nome)+')" title="Filtrar por '+esc(x.nome)+'" style="border:0;background:'+(sel?'var(--orange-soft)':'transparent')+';cursor:pointer;border-radius:12px;padding:4px"><div class="dash-column-value-row"><span>'+x.enderecos.size+'</span><span>'+x.contagens+'</span></div><div class="dash-column-bars"><i class="dash-column-bar audited" style="height:'+he+'%"></i><i class="dash-column-bar" style="height:'+hc+'%;background:#f97316"></i></div><strong class="dash-column-label">'+esc((i+1)+'º '+x.nome)+'</strong><small>'+x.produtos.size+' produto(s)</small></button>';}).join('')+'</div></div>';
    ...
    const barrasHora=horas.map(function(x){const alt=Math.max(8,Math.round(x.n/maxHora*100)),ativo=x.n>0,sel=produtividadeHoraSelecionada===x.h;return '<button type="button" onclick="filtrarProdutividadeHora('+x.h+')" title="'+String(x.h).padStart(2,'0')+':00 · '+x.n+' contagem(ns)" style="display:flex;flex-direction:column;align-items:center;gap:6px;flex:1;min-width:22px;border:0;background:'+(sel?'var(--orange-soft)':'transparent')+';cursor:pointer;border-radius:10px;padding:5px 2px"><div style="height:110px;width:100%;display:flex;align-items:flex-end;justify-content:center"><div style="width:100%;max-width:22px;height:'+(ativo?alt:8)+'%;border-radius:8px 8px 4px 4px;background:'+(ativo?'linear-gradient(180deg,var(--orange-h),var(--orange))':'var(--border)')+'"></div></div><div style="font-size:.61rem;font-family:var(--mono);color:'+(sel?'var(--orange)':'var(--muted)')+'">'+String(x.h).padStart(2,'0')+'</div></button>';}).join('');
