...
function onGtinInput() {
    ...
    if (APP.base.length) {
        ...
        var regInventario = APP.base.find(_match);
        var produtoGeral = ((_a = window.DTProdutos) === null || _a === void 0 ? void 0 : _a.buscarSync(val)) || { encontrado: false };
        var reg = regInventario || (produtoGeral.encontrado ? {
            codigo_produto: produtoGeral.codigoInterno || produtoGeral.produtoId || valNorm_1,
            descricao_produto: produtoGeral.nomeProduto || 'Produto sem descrição',
            gtin: produtoGeral.gtin || valNorm_1,
        ...
        if (reg) {
            ...
            var registrosEndereco = endNorm_1 ? APP.base.filter(function (r) { return r._end === endNorm_1; }) : [];
            var baseEnderecoDisponivel = registrosEndereco.length > 0;
            var pertenceAoEnd = baseEnderecoDisponivel && registrosEndereco.some(_match);
            APP.atual.produtoDivergenteEnd = !!(endNorm_1 && baseEnderecoDisponivel && !pertenceAoEnd);
            APP.atual.validacaoEnderecoPendente = !!(endNorm_1 && !baseEnderecoDisponivel);
            ...
            pbox.style.display = '';
            var nomeProduto = produtoGeral.encontrado
                ? produtoGeral.nomeProduto
                : (reg.descricao_produto || 'Produto sem descrição');
            var codigoCadastro = produtoGeral.encontrado
                ? (produtoGeral.codigoInterno || reg.codigo_produto || '')
                : (reg.codigo_produto || '');
            pbox.innerHTML = "\n        <div class=\"prod-card\">\n          <div class=\"prod-icon\">\uD83D\uDCE6</div>\n          <div style=\"min-width:0\">\n            <div class=\"prod-code\" style=\"font-size:.82rem;font-weight:800\">".concat(escHTML(isDun ? 'DUN' : 'GTIN'), ": ").concat(escHTML(val)).concat(codigoCadastro ? " \u00B7 C\u00F3d. ".concat(escHTML(codigoCadastro)) : '', "</div>\n            <div class=\"prod-name\" style=\"font-size:.7rem;font-weight:600;margin-top:3px;line-height:1.25;color:var(--muted)\">").concat(escHTML(nomeProduto), "</div>\n          </div>\n        </div>");
        }
        else {
            APP.atual.produtoAtual = null;
            APP.atual.produtoDivergenteEnd = false;
            var produtosCarregados = !!((_c = (_b = window.DTProdutos) === null || _b === void 0 ? void 0 : _b.cache) === null || _c === void 0 ? void 0 : _c.carregado);
            ...
            pbox.style.display = 'none';
            if (!produtosCarregados && ((_d = window.DTProdutos) === null || _d === void 0 ? void 0 : _d.carregar)) {
                var codigoConsultado_1 = val;
                window.DTProdutos.carregar().then(function () {
                    var campo = document.getElementById('f-gtin');
                    if (campo && campo.value.trim() === codigoConsultado_1)
                        onGtinInput();
                }).catch(function () { });
            }
...
function _executarSalvar(qty) {
    ...
    if (APP.modoRecontagem) {
        var _endRec = a._endNorm || a.endereco || APP.modoRecontagem.endereco || '';
        var _capRec = Number((_x = (_w = (_v = (_u = (_t = (_s = APP.endCapacidade) === null || _s === void 0 ? void 0 : _s[_endRec]) !== null && _t !== void 0 ? _t : a.capacidadeEnd) !== null && _u !== void 0 ? _u : APP.modoRecontagem.capacidade_pallets) !== null && _v !== void 0 ? _v : APP.modoRecontagem.capacidade_paletes) !== null && _w !== void 0 ? _w : APP.modoRecontagem.capacidade) !== null && _x !== void 0 ? _x : 0) || 0;
        var _usadosRec = _palletsNoEnderecoAtual(_endRec);
        ...
        if (_capRec > 0 && _usadosRec >= _capRec) {
            toast("\u2705 Recontagem concluida: ".concat(_usadosRec, "/").concat(_capRec, " paletes registrados"), 's');
            resetContagem();
            _concluirRecontagem();
            return;
        }
        ...
        var faltam = _capRec > 0 ? Math.max(0, _capRec - _usadosRec) : null;
        toast(faltam === null
            ? "\uD83D\uDCE6 Palete ".concat(_usadosRec, " salvo \u2014 bipe o proximo ou encerre o endereco")
            : "\uD83D\uDCE6 Palete ".concat(_usadosRec, "/").concat(_capRec, " salvo \u2014 faltam ").concat(faltam), 's');
        return;
    }
    // ── Verificar capacidade após salvar ─────────────────────────────────
    // Re-ler capacidade diretamente do mapa (mais confiável que APP.atual.capacidadeEnd)
    var _capMapa = (_0 = (_z = (_y = APP.endCapacidade) === null || _y === void 0 ? void 0 : _y[a._endNorm]) !== null && _z !== void 0 ? _z : a.capacidadeEnd) !== null && _0 !== void 0 ? _0 : 0;
    var _cap = _capMapa;
