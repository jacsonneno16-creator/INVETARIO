...
function _atualizarBadgeRecontagens() {
    ...
    var recs = (APP.recontagens || []).filter(function (r) {
        ...
        if (st === 'PERSISTENTE' || (r.status_bloqueio || '') === 'PERSISTENTE_BLOQUEADO')
            return false;
        if (st === 'CONCLUIDA' || stRec === 'concluida' || stRec === 'aguardando_analista')
            return false;
    ...
    try {
        ...
        _unsubRec = FS.collection('dt_recontagens')
            ...
            APP.recontagens.forEach(function (r) {
                ...
                var chaveFluxo = window.InventoryFlowKey
                    ? window.InventoryFlowKey.chave(r, APP.inventarios || (APP.inventario ? [APP.inventario] : []))
                    : "".concat(invId, "|").concat(String(r.endereco).trim().toUpperCase(), "|").concat(r.produto || r.codigo_produto || 'SEM_PRODUTO');
                var chave = encodeURIComponent(chaveFluxo);
...
function _itemAguardandoAnalista(item) {
    ...
    if (stRec === 'aguardando_analista')
        return true;
    if (stRec === 'concluida' || status === 'CONCLUIDA')
        return true;
...
function selecionarEnderecoRecontagem(itemId) {
    ...
    var numRec = Number(item.numero_recontagem || 1);
    var terceiraJaRealizada = item.qtd_terceira != null || numRec > 2;
...
function _ativarModoRecontagem(item) {
    ...
    if (!itemNorm.divergencia_id || itemNorm.id === itemNorm.divergencia_id) {
        ...
        var divId_1 = itemNorm.divergencia_id || item.id;
        itemNorm.divergencia_id = divId_1;
        // Buscar recontagem pendente vinculada para obter o id correto
        var recVinculada = (APP.recontagens || [])
            .filter(function (r) {
            if (String(r.divergencia_id || '') !== String(divId_1))
                return false;
            var status = String(r.status || '').toUpperCase();
            var stRec = String(r.status_recontagem || 'pendente').toLowerCase();
            if (['CONCLUIDA', 'CANCELADA', 'PERSISTENTE', 'RESOLVIDA'].includes(status))
                return false;
            if (['concluida', 'cancelada', 'persistente', 'resolvida', 'aguardando_analista'].includes(stRec))
                return false;
            return status === 'PENDENTE' || stRec === 'pendente';
        })
            .sort(function (a, b) { return Number(a.numero_recontagem || 1) - Number(b.numero_recontagem || 1); })[0];
        if (recVinculada) {
...
function _concluirRecontagem() {
    ...
    if (navigator.onLine && item.id) {
        ...
        var upd = {
            ...
            operador_recontagem: ((_a = APP.operador) === null || _a === void 0 ? void 0 : _a.name) || '',
            status: 'CONCLUIDA'
        };
        // Garantir que estamos atualizando o documento de RECONTAGEM e não a DIVERGÊNCIA
        var idRecontagemReal = (item._col === 'recontagem') ? item.id : (item.divergencia_id && item.id !== item.divergencia_id ? item.id : null);
        if (idRecontagemReal) {
            FS.collection('dt_recontagens').doc(idRecontagemReal).update(upd).catch(function (e) { return console.warn('[Rec]', e.message); });
        }
