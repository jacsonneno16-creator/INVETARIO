...
function _carregarBaseGeralProdutosInventario() {
    return __awaiter(this, arguments, void 0, function (force) {
        ...
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    if (!((_a = window.DTProdutos) === null || _a === void 0 ? void 0 : _a.carregar))
                        return [2 /*return*/, []];
                    return [4 /*yield*/, window.DTProdutos.carregar(force)];
                case 1:
                    produtos = _b.sent();
                    total = (produtos || []).filter(function (p) { return p && p.ativo !== false; }).length;
                    if (!(!total && navigator.onLine && !force)) return [3 /*break*/, 3];
                    return [4 /*yield*/, window.DTProdutos.carregar(true)];
                case 2:
                    produtos = _b.sent();
                    total = (produtos || []).filter(function (p) { return p && p.ativo !== false; }).length;
                    _b.label = 3;
                case 3: return [2 /*return*/, produtos || []];
            }
        });
    });
}
function selecionarInventario(id_1) {
    return __awaiter(this, arguments, void 0, function (id, modo) {
        var inv, metaLocal, verLocal, verServidor, baseLocal, capLocal, cacheValido, produtosGerais, capMapa_1, cached, locaisMap, cs;
        if (modo === void 0) { modo = 'inventario'; }
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    APP.modoPendente = modo || 'inventario';
                    inv = (APP.inventariosDisponiveis || []).find(function (i) { return i.id === id; });
                    if (!inv) {
                        toast('Inventário não encontrado', 'e');
                        return [2 /*return*/];
                    }
                    metaLocal = baseMetaLoad(id);
                    verLocal = bVerLoad(id);
                    verServidor = _invBaseVer(inv);
                    baseLocal = metaLocal ? baseLoad(id) : null;
                    capLocal = endCapLoad(id);
                    cacheValido = (baseLocal &&
                        baseLocal.length > 0 &&
                        // Cache é válido se a versão bate OU se não há versão para comparar
                        (verServidor === '' || verLocal === verServidor));
                    if (!cacheValido) return [3 /*break*/, 2];
                    return [4 /*yield*/, _carregarBaseGeralProdutosInventario(false)];
                case 1:
                    produtosGerais = _a.sent();
                    ...
                    APP.inventario = inv;
                    APP.base = baseLocal;
                    capMapa_1 = _recalcularEndCap(baseLocal);
                    try {
                        cached = localStorage.getItem(LS_LOCAIS);
                        if (cached) {
                            locaisMap = JSON.parse(cached);
                    ...
                    try {
                        cs = localStorage.getItem(LS_LOCAIS + '_set');
...
function _executarDownload(inv) {
    return __awaiter(this, void 0, void 0, function () {
        var invId, baseLocal, produtosGeraisOffline, docData, snap, e_2, produtosGerais, nChunks, baseRaw, cSnap, total, i, itens, pct, e_3, baseNorm, endCapMapa, _forceLocaisRefresh, versaoServidor, metaDoc, e_4, versaoLocal, _cacheValido, raw, rawSet, motivo, locaisSet_1, chunksSnap, docsUsar, e_5, raw, rawSet, comLimite;
        var _a, _b;
        return __generator(this, function (_c) {
            switch (_c.label) {
                case 0:
                    ...
                    if (!(!navigator.onLine && baseLocal && baseLocal.length > 0)) return [3 /*break*/, 2];
                    return [4 /*yield*/, _carregarBaseGeralProdutosInventario(false)];
                case 1:
                    produtosGeraisOffline = _c.sent();
                ...
                case 7:
                    ...
                    baseRaw = [];
                    if (!(nChunks > 0)) return [3 /*break*/, 12];
                    _c.label = 8;
                case 8:
                ...
                case 17:
                    ...
                    if (!_cacheValido) return [3 /*break*/, 18];
